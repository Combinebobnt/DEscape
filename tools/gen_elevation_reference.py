...
DOC_HINTS = {
    "lone_bump": (
        "Look at the single raised tile from directly above (top-down) if possible: "
        "does it form a symmetric cone/pyramid, or something else?"
    ),
    ...
    "staircase": (
        "A single point raised straight to elevation 7 -- MapManager's own propagation "
        "built the surrounding rings (see the generator's docstring). Compare "
        "ring-by-ring against this tool's render: same ring radii, same px-per-level "
        "spacing?"
    ),
    "plateau": (
        "Check a convex (outer) corner of the 8x8 raised square: does DE's ramp there "
        "read as the corner's max height, min height, or an average?"
    ),
    ...
    "steep_seam": (
        "An intentionally illegal Delta=2 seam (this tool never lets its own edit tools "
        "create one) -- does DE render it as a steep double-ramp, clamp it, or silently "
        "repair it back to Delta=1?"
    ),
}
